...
import { DEMO_PROFILE_DATA, getDemoUserProfile } from '../constants/demo';
...
export const ProfileService = {
  ...
  /**
   * Initializes or returns the demo user profile.
   * Does not create duplicate profiles if an onboarding-completed profile exists.
   */
  saveDemoProfile: async (userId: string, fullName: string = 'Demo Athlete'): Promise<UserProfile> => {
    try {
      ...
      const demoProfile = getDemoUserProfile(userId, fullName);
        ...
        try {
          await supabase
            ...
            .upsert({
              ...
              age: demoProfile.age,
              sex: demoProfile.sex,
              height_cm: demoProfile.height_cm,
              weight_kg: demoProfile.weight_kg,
              fitness_goal: demoProfile.fitness_goal,
              lifestyle_activity: demoProfile.lifestyle_activity,
              diet: demoProfile.diet,
              accessibility_needs: demoProfile.accessibility_needs,
              has_exercise_restriction: demoProfile.has_exercise_restriction,
              strength_equipment: demoProfile.strength_equipment,
              strength_experience: demoProfile.strength_experience,
              onboarding_completed: true,
              updated_at: new Date().toISOString(),
            }, { onConflict: 'user_id' });
        } catch (err) {
          console.warn('Supabase demo profile upsert warning (table may not be migrated yet):', err);
        }
      }
      ...
      console.warn('Failed to save demo profile:', err);
      const demoProfile = getDemoUserProfile(userId, fullName);
      await SafeStorage.setItem(
        `${PROFILE_STORAGE_KEY_PREFIX}${userId}`,
        JSON.stringify(demoProfile)
