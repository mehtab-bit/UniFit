...
def get_weekly_workout_plan(
    ...
    try:
        ...
        return WeeklyWorkoutPlanResponse(**workout_plan)
    except Exception as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc


@router.get("/{date}", response_model=WorkoutDaySchema)
def get_workout_by_date(
    ...
    try:
        ...
        # Check cached weekly plan first
        cached = supabase_service.get_cached_weekly_plan(user_id, week_number)
        if cached and "workouts" in cached:
            for day in cached["workouts"]:
                if day["day"].lower() == target_day_name:
                    return WorkoutDaySchema(**day)

        # Fallback: generate default week 1 workout plan
        workout_plan = engine_service.generate_weekly_workout(
            lifestyle="light",
        ...
        raise HTTPException(status_code=404, detail=f"No workout scheduled for day {target_day_name}")
